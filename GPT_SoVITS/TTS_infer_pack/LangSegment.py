...
class LangSegment():
    ...
    def _saveData(words, language: str, text: str):
        ...
        if not "|" in language: lang_count[language] += int(
            len(text) // 2) if language == "en" else len(text)
    ...
    def _addwords(words, language, text):
        ...
        if preResult and ("|" in preResult["lang"]):
            ...
            if ispre_waits: preResult = LangSegment._saveData(words, preResult[
                "lang"], preResult["text"])
        pre_lang = preResult["lang"] if preResult else None
        if ("|" in language) and (
                pre_lang and not pre_lang in language and not "…" in language): language = \
        language.split("|")[0]
        filters = LangSegment.Langfilters
        if "|" in language:
    ...
    def _parse_language(words, segment):
        ...
        for index, text in enumerate(lines):
            ...
            nextPunc = len(re.sub(regex_pattern, '',
                                  re.sub(r'\n+', '', nextText)).strip()) == 0
            textPunc = len(re.sub(regex_pattern, '',
                                  re.sub(r'\n+', '', text)).strip()) == 0
            if not EOS and (textPunc == True or (
                    len(nextText.strip()) >= 0 and nextPunc == True)):
            ...
            if len(cleans_text) <= 3 and LangSegment._is_chinese(cleans_text):
                if EOS and LANG_EOS:
                    ...
                    if match_char in "。.":
                        language = prev_language if referen and len(
                            words) > 0 else language
                    else:
    ...
    def _parse_symbols(text):
        ...
        process_list = [
            (TAG_S1, re.compile(LangSegment.SYMBOLS_PATTERN),
             LangSegment._process_symbol),
            (TAG_KO, re.compile(
                '(([【《（(“‘"\']*(\d+\W*\s*)*[\uac00-\ud7a3]+[\W\s]*)+)'),
             LangSegment._process_korean),
            (TAG_NUM, re.compile(r'(\W*\d+\W+\d*\W*\d*)'),
             LangSegment._process_number),
            (TAG_EN, re.compile(r'(([【《（(“‘"\']*[a-zA-Z]+[\W\s]*)+)'),
             LangSegment._process_english),
            (TAG_P1, re.compile(r'(["\'])(.*?)(\1)'),
    ...
    def getCounts():
        ...
        for d in text_langs: lang_counts[d['lang']] += int(
            len(d['text']) // 2) if d['lang'] == "en" else len(d['text'])
        lang_counts = dict(
